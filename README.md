...
| SQL & Databases | Queries, joins, aggregations, window functions |
...
Combined with your quiz scores and activity history, this creates a learner profile that can drive adaptive content recommendations.
...
| `POST` | `/api/auth/logout` | Y | Invalidate token |
| `GET` | `/api/courses/topics` | Y | Full topic catalog |
| `POST` | `/api/learning/activity` | Y | Record an activity |
| `GET` | `/api/learning/activities` | Y | Your activity history |
| `POST` | `/api/learning/feedback` | Y | Submit topic feedback |
| `GET` | `/api/learning/feedback` | Y | Your feedback history |
...
- [ ] **LLM integration** - use proficiency + style to generate adaptive explanations via Claude or GPT
